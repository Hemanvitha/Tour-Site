...
const users = require('../tourServer/userTourHash.json');

let tourDB = require('./tourDBRef');
let usersDB = require('./usersDBRef');
var toursInfo = tourDB.insert(tours);
var app = express();
...
const cookieName = "UD4558toursid";
app.use(session({
    secret: 'Tom',
    resave: false,
...
var getTours = async function (req, res) {
    ...
    var toursData = await tourDB.insert(tours);
    res.json(toursData);
    res.status(200);
}
...
var getTourByID = async function (req, res) {
    ...
    console.log(tourID);
    var tour = await toursInfo.find({ "_id": tourID });
    if ( tour.length > 0) {
